...
audio_hal_func_t AUDIO_CODEC_WM8978_DEFAULT_HANDLE = {
    ...
    .audio_codec_set_mute = wm8978_mute,
...
// static esp_err_t wm_write_reg(uint8_t reg_add, uint8_t data)
// {
//     return i2c_bus_write_bytes(i2c_handle, WM8978_ADDR, &reg_add, sizeof(reg_add), &data, sizeof(data));
// }

// static esp_err_t wm_read_reg(uint8_t reg_add, uint8_t *p_data)
// {
//     return i2c_bus_read_bytes(i2c_handle, WM8978_ADDR, &reg_add, sizeof(reg_add), p_data, 1);
// }

static int wm8978_write_reg(uint8_t reg, uint16_t val)
{
    // return wm_write_reg(reg, val);
    int res = 0;
    uint8_t buf[2];
	buf[0]=((val&0x0100)>>8)|(reg<<1);
	buf[1]=(uint8_t)(val&0xff);
	res=(int)_i2c_master_mem_write(I2C_NUM_0,WM8978_ADDR,buf[0],buf+1,1);
